...
def extract_study_summaries(disease_name, search_query):
    ...
    # Initialize list to store all summary data
    all_summaries = []
        ...
        try:
            ...
            for study in studies:
                ...
                if found_match:
                    ...
                    # Extract the description module
                    ...
                    # Extract the results section of the study
                    results_section = study.get('resultsSection', {})
                    
                    # Extract primary outcomes from the results
                    primary_outcomes = results_section.get('primaryOutcomes', []) if results_section else []
                    # Initialize string to store primary outcome information
                    primary_outcome_str = ''
                    # Check if primary outcomes exist
                    if primary_outcomes:
                        ...
                        for outcome in primary_outcomes:
                            ...
                            # Get the time frame for the outcome
                            time_frame = outcome.get('timeFrame', 'N/A')
                            # Append outcome information to the string
                            primary_outcome_str += f"{measure} | {description} | TimeFrame: {time_frame}; "
                        # Strip trailing spaces from the outcome string
                        primary_outcome_str = primary_outcome_str.strip()
                    else:
                        # Set to N/A if no primary outcomes found
                        primary_outcome_str = 'N/A'
                    
                    # Extract secondary outcomes from the results
                    secondary_outcomes = results_section.get('secondaryOutcomes', []) if results_section else []
                    # Initialize string to store secondary outcome information
                    secondary_outcome_str = ''
                    # Check if secondary outcomes exist
                    if secondary_outcomes:
                        # Process each secondary outcome
                        ...
                        # Set to N/A if no secondary outcomes found
                        secondary_outcome_str = 'N/A'
                    
                    # Create a dictionary with all summary information
    ...
    if all_summaries:
        ...
        # Count the initial number of summaries
        initial_count = len(df)
        
        # Remove duplicate summaries based on NCTId
        ...
        # Count the final number of summaries
        final_count = len(df)
        ...
        # Log if no summaries were found
...
def etl_summaries(event, context):
    ...
    try:
        ...
        for disease_name, search_query in DISEASES.items():
            # Extract summaries for this disease
        ...
        # Log the total number of summaries extracted
        ...
        # Log the number of summaries after filtering
        ...
        # Log that text fields have been cleaned
        ...
        for idx, row in df_combined.iterrows():
            try:
                # Upsert the row into the clinical_trials_summary table
        ...
        # Log the number of successfully uploaded summaries
